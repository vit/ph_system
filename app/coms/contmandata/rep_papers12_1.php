...
<br>
<center><span class=subtitle>Report: Author's section changes</span></center>
<p>
...
<tr class=$cellclass2>
<th colspan=5>Authors (text)</th>
</tr>
<tr class=$cellclass2>
<th colspan=5>Authors (with PINs)</th>
</tr>
<tr class=$cellclass2>
...
<tr class=cell8>
<th colspan=5>Final decision and section</th>
</tr>
<tr class=cell4>
<th colspan=5>Editor's recommendations: score, decision, section, comments</th>
</tr>
<tr class=cell6>
<th colspan=5>Reviewer's recommendations: score, decision, section, comments</th>
</tr>
...
FROM 
 (editor AS e INNER JOIN paper AS p ON e.userpin=$USERPIN AND e.context=$CURRENTCONT AND e.editorid=p.editor
 )
 LEFT JOIN 
...
FROM 
  (paper AS p LEFT JOIN editor AS e ON p.editor=e.editorid)
    LEFT JOIN 
      review AS r ON p.papnum=r.papnum AND r.context=$CURRENTCONT
  WHERE p.context=$CURRENTCONT AND (p.subject != p.ed_subject)
...
<tr class=$cellclass>
<td colspan=5><b>Authors (text):</b> $cauthors1</td>
</tr>
<tr class=$cellclass>
<td colspan=5><b>Authors (with PINs):</b> $cauthors2</td>
</tr>
<tr class=$cellclass>
<td colspan=5><b>Main:</b> $ckeywords1; <b>Secondary:</b> $ckeywords2</td>
</tr>
<tr class=cell3>
