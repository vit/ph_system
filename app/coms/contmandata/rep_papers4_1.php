...
$scoretitle = $SCORES[$score];


$scorecond2 = "";
if(!$score>0) {
  $score=0;
  $scorecond2 = "OR p.ed_score IS NULL";
}
...
FROM 
  paper AS p
  WHERE p.context=$CURRENTCONT AND (p.ed_score=$score $scorecond2)
");
...
<br>
<center><span class=subtitle>Report: Papers with editor's score <i>"$scoretitle"</i> (detailed)<br>Found $n papers</span></center>
<p>
...
<tr class=cell6>
<th colspan=3>Reviewer's recommendations: score, decision, section, comments</th>
</tr>
...
FROM 
  ...
  WHERE p.context=$CURRENTCONT AND (p.ed_score=$score $scorecond2)
ORDER BY p.ed_recommendation, p.ed_subject, p.papnum, r.revpin
");
